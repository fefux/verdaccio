...
Storage.prototype.size = function(callback) {
  ...
  if (!storage) return callback( Error[404]('No storage available') )
  FolderSize(this.config.storage, callback);
}

Storage.prototype.add_cached = function(name) {
  this.config.localList.add_cached(name);
}
